...
class PreTrain(torch.nn.Module):
    ...
    def train_graphcl(self, model, loader1, loader2, optimizer):
        ...
        for step, batch in enumerate(zip(loader1, loader2)):
            ...
            # Calculate Accuracy and F1 Score for the current batch
            if batch1.y is not None:  # Ensure batch1.y is not None before proceeding
                preds = torch.argmax(x1, dim=1)  # Get predictions from the model output
                accuracy_metric.update(preds, batch1.y.to(device))  # Update accuracy metric
                f1_metric.update(preds, batch1.y.to(device))  # Update F1 score metric

        accuracy = accuracy_metric.compute()
        f1_score = f1_metric.compute()

        print(f'Training Accuracy: {accuracy:.4f}, F1 Score: {f1_score:.4f}')
